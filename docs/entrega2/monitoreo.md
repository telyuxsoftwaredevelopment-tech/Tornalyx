...
| mysqld_exporter | 9104 | Conexiones activas, queries lentas, tamaño de tablas, replicación (si la hubiera) | `tornalyx_monitor` (usuario `tornalyx_monitor` en `SGDM/backend/database/dcl.sql`): solo `SELECT` + `PROCESS`, no puede modificar datos. |
